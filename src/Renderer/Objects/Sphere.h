...
namespace renderer{
    ...
    class Sphere : public ObjectBase<SphereVertex>{
    ...
    public:
        Sphere(const char* name,
             ...
             vec3 color = vec3(-1))
                : ObjectBase(name, mode, type, color), m_sphere(_sphere)
        {}
        ...
        void SetupVerticesBuffer() override {
            ...
            for (int i = 0; i <= stacks; ++i) {
                float latitude = glm::pi<float>() * (-0.5f + static_cast<float>(i) / stacks);
                float sinLat = std::sin(latitude);
                float cosLat = std::cos(latitude);

                for (int j = 0; j <= slices; ++j) {
                    float longitude = 2 * glm::pi<float>() * static_cast<float>(j) / slices;
