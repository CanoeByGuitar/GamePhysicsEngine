...
void Cloth::SetupEdgeList() {
    for (const auto& mesh : m_model->m_meshes) {
        ...
        m_vertices.insert(m_vertices.end(), mesh.vertices.begin(),
                          mesh.vertices.end());
    ...
    PHY_INFO(
        "Models have {} meshes, there are totally {} vertices and {} indices.",
        m_model->m_meshes.size(), m_vertices.size(), m_indices.size());

    // {[vert0, vert1, triangleID]}
    std::vector<std::vector<unsigned int>> TripleList(
        m_indices.size(), std::vector<unsigned int>(3));
    ...
    sort(TripleList.begin(), TripleList.end(),
         [](std::vector<unsigned int> lhs, std::vector<unsigned int> rhs) {
             if (lhs[0] == rhs[0]) {
                 if (lhs[1] == rhs[1]) {
                     return lhs[2] < rhs[2];
                 } else {
    ...
    while (i < TripleList.size()) {
        if (TripleList[i][0] == temp0 && TripleList[i][1] == temp1) {
            ...
            m_edgeList.emplace_back(TripleList[i][0], TripleList[i][1]);
...
void Cloth::Init() {
    ...
    m_originVelocities = m_velocities;
    control::restart = true;
    
    // find fixed points;
    auto bound = GetBound(*m_model);
    control::scene_bound = bound;
    float y_max = GetMax(bound).y;
    float y_min = GetMin(bound).y;
    for(int i = 0; i < m_vertices.size(); i++){
        if(m_vertices[i].y >= y_max - 0.1f * (y_max - y_min)){
            m_fixedIndices[i] = 1;
        }
    }
}
...
void Cloth::AdvanceOneSubstep() {
    if (m_clothScheme == ImplicitSolver) {
        ...
        for (int i = 0; i < m_vertices.size(); i++) {
            ...
            init_x[i] =
                m_vertices[i] + m_dt * m_velocities[i];  // x0 = x0 + dt * v0
        }

        for (int k = 0; k < m_iter; k++) {
            auto negGradient = SolveLinearSystem();
            for (int i = 0; i < m_vertices.size(); i++) {
                if (m_fixedIndices.find(i) != m_fixedIndices.end())
                    continue;
                m_vertices[i] +=
                    negGradient[i] / (m_mass / (m_dt * m_dt) + 4.f * m_springK);
            }
        }
...
std::vector<vec3> Cloth::SolveLinearSystem() {
    ...
    for (int i = 0; i < m_vertices.size(); i++) {
        ...
        g[i] = -m_mass *
                   (m_vertices[i] - (last_x[i] + m_dt * m_velocities[i])) /
                   (m_dt * m_dt) +
               m_mass * m_gravity + m_mass * m_wind;
        ...
        for (int e = 0; e < m_edgeList.size(); e++) {
            ...
            vec3 f =
                -m_springK * (1 - m_L[e] / glm::length(pa - pb)) * (pa - pb);
            // fa = fa + f  fb = fb - f
            // ga = ga + fa  gb = gv - fb
            g[a] = g[a] + f;
            g[b] = g[b] - f;
...
void Cloth::HandleCollision() {
    ...
    for (int i = 0; i < m_vertices.size(); i++) {
        if (glm::dot(m_vertices[i] - center, m_vertices[i] - center) <
            radius * radius) {
            m_velocities[i] +=
                0.2f *
                (center + radius * glm::normalize(m_vertices[i] - center) -
                 m_vertices[i]) /
                m_dt;
            m_vertices[i] =
                center + radius * glm::normalize(m_vertices[i] - center);
...
void Cloth::Restart() {
    ...
    m_velocities = m_originVelocities;
}
